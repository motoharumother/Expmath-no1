...
plt.plot(t, error, label='ExEuler', alpha=0.8)
y = Runge_kutta(y, dt, t)
error = (np.log(y[0,:])-y[0,:])-(y[1,:]-2*np.log(y[1,:]))-(np.log(2)-3)
plt.plot(t, error, label='Runge_Kutta', alpha=0.8)

y = SympleticEuler(y, dt, t)
error = (np.log(y[0,:])-y[0,:])-(y[1,:]-2*np.log(y[1,:]))-(np.log(2)-3)
plt.plot(t, error, label='SympleticEuler', alpha=0.8)

y = ImEuler(y, dt, t)
error = (np.log(y[0,:])-y[0,:])-(y[1,:]-2*np.log(y[1,:]))-(np.log(2)-3)
plt.plot(t, error, label='ImEuler', alpha=0.8)
y = MidpointRule(y, dt, t)
error = (np.log(y[0,:])-y[0,:])-(y[1,:]-2*np.log(y[1,:]))-(np.log(2)-3)
plt.plot(t, error, label='MidpointRule', alpha=0.8)
